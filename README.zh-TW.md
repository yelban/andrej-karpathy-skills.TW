...
回覆會給你成功條件（例如「Playwright 截圖比對 10 次、位移 < 2px」）、驗證指令、明確非目標。若任務太主觀或太小，會回「不適用，建議直接做」、不硬轉換。適合單一 prompt 套用宣告式紀律、不需要自主迭代的場合。
...
```
1. /dec <模糊需求>                 ← 改寫成契約（成功條件 + 驗證指令 + 非目標）
2. 你 review 契約                  ← 人類確認方向
3. /goal "<success + 非目標>"      ← Haiku 接手當判官
