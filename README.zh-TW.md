...
| **D. `git clone` + `ln -s`** | symlink 整檔 *或* `sed` 追加規則 | `/dec`（短、symlink 過去） | 透過 `git pull` 自動更新 |
...
**選項 D：`git clone` + symlink** — 單一管理來源、`git pull` 一鍵更新、不過 marketplace、不帶 namespace。
...
# 之後要更新（拿到新版 /dec 輸出格式、README、EXPERIMENT.md⋯⋯）：
...
- **只裝 D**（推薦給 power user）— clone 一次、symlink 完成、之後 `git pull` 更新。單一管理來源、不依賴 marketplace、保留短 `/dec`。
